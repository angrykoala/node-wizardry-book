...
These modules are already installed by default with Node, so they don't have to be downloaded or installed. The API may be different depending on the version of Node you are using, remember to check the official documentation (https://nodejs.org/api) to see the current specification.
